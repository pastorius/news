...
const { toMarkdown, buildPrompt, htmlBodyFromMultipart } = require('../../../src/summarize/parse');
const fs = require('fs').promises
const path = require('path')

describe('with expected multipart', () => {
  describe('#toMarkdown', () => {
    it('should convert html to markdown', async () => {
        const result = await toMarkdown(await fs.readFile(path.join(__dirname, '../../fixtures/email.01.txt'), { encoding: 'utf8' }))
        assert.equal(result, await fs.readFile(path.join(__dirname, '../../fixtures/email.01.expected.txt'), { encoding: 'utf-8' }))
    });
  });
